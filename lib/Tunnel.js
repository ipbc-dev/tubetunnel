...
const debug = require('debug')('tubetunnel:client');
...
module.exports = class Tunnel extends EventEmitter {
  constructor(opts = {}) {
    ...
    this.closed = false;
    if (!this.opts.host) {
      this.opts.host = 'https://tunnel.bittube.app';
    }
  }
    ...
    (function getUrl() {
      axios
        ...
        .then(res => {
          ...
          if (res.status !== 200) {
            const err = new Error(
              (body && body.message) || 'tubetunnel server returned an error, please try again'
            );
            return cb(err);
          }
  ...
  _establish(info) {
    // increase max event listeners so that tubetunnel consumers don't get
